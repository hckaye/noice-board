 ...
 */
export const isPostContentEqual = (
  postContent1: PostContent,
  postContent2: PostContent,
): boolean => {
  return (
    getPostContentValue(postContent1) === getPostContentValue(postContent2)
  );
};
 ...
 * PostContentの行数を取得する
 */
export const getPostContentLineCount = (postContent: PostContent): number => {
  const content = getPostContentValue(postContent);
  if (content === "") return 0;
